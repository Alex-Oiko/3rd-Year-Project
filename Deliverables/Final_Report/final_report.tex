...
OpCodes in this context stands for operation codes. As in what operation needs to be done at some point. If the program flow is looked thoroughly, it is apparent that some nodes need to do different computations at different points of the algorithm. For example, a $p$ node needs to either store the value of its respective $r$ node in its value, or do a computation such as $\alpha *p$
