...
void Simulate::LoadFireAll(Task& TASK, Dealer& DEAL, Core& CORE){
    ...
    for(int node = 0; node < TASK.nextPID; node++){ //report initial state
        ...
        TTE = iTTE->second;
        if(TTE.OpCodes.front() == 3){
            E.Value = TTE.IV;
            E.Kd = E.Ks;
	    CORE.mop[TTE.V].push(CORE.mop[TTE.V].front());
	    CORE.mop[TTE.V].pop();
            EventQ.push(E);
        }
	else if(TTE.OpCodes.front()==2){
    		matrix_size=TTE.YD;
	}
...
void Simulate::Deliver(event E, Core& CORE){    //this is the MC packet arrival interrupt;
    ...
    for(int v = 0; v < vTTE.size(); v++){
        ...
        RES.Type = FIREAWAY;
        RES.OutLink = 6;
	switch(CORE.mop[TTE.V].front()){
		case 0:
	    		CORE.mop[TTE.V].push(CORE.mop[TTE.V].front());
	    		CORE.mop[TTE.V].pop();
			cout<<"opcode is 0 here"<<endl;
			cout<<"new opcode is "<<CORE.mop[TTE.V].front()<<endl;
			break;
		...
		case 6://to create Ap nodes
			...
			CORE.Mcounter[TTE.V]++;
			CORE.Mstore[TTE.V]+=E.Value;
			cout<<"Opcode"<<CORE.mop[TTE.V].front()<<endl;
	    		CORE.mop[TTE.V].push(CORE.mop[TTE.V].front());
	    		CORE.mop[TTE.V].pop();
			if(CORE.Mcounter[TTE.V]==TTE.YD){
				RES.Value=CORE.Mstore[TTE.V];
